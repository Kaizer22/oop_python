"""Инвентарь бойца в котором распологаются предметы,
    усиливающие его характеристики """
...
class Inventory:
    ...
    # Подсчет общего влияния всех предметов на показатели бойца
    ...
    # Каждый ход предметы постепенно приходят в негодность и могут сломаться
    # и пропасть из инвентаря
    def update_items_strength(self):
        ...
        for item in self.items:
            ...
            if item.strength < 0:
                ...
                print("Сломан предмет: {0}".format(item.name))
